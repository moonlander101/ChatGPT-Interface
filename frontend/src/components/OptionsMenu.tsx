...
function OptionsMenu({ showOptions, reference, chat } : { showOptions: boolean, reference: React.RefCallback<HTMLDivElement>, chat : SidebarButtonProps }) {
    ...
    const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.stopPropagation();
        console.log("[OptionsMenu handleDelete] Clicked buttont to delete bruh")
        openDeleteModal({id : chat.id, title : chat.title });
    }

    const handleRenameClick = () => {
        // e.stopPropagation();
        console.log("[OptionsMenu handleRename] Clicked button to rename bruh")
    }
    ...
    return ( 
        <>
            <div className="z-20 p-1 absolute w-auto h-auto bg-[#2f2f2f] right-3 translate-y-16 rounded-xl border-[1px]  border-[#414141]" hidden={!showOptions} ref={reference}>
                <button className="flex w-28 h-10 hover:backdrop-brightness-150 rounded-md items-center p-1" onClick={handleRenameClick}>
                ...
                <button className="flex w-28 h-10 hover:backdrop-brightness-150 rounded-md items-center p-1 text-[#ef3936]" onClick={(e)=>{handleClick(e)}}>
